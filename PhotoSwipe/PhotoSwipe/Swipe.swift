            ...
            ScrollView{
                LazyVGrid(columns: columns, spacing: 10){
                    ForEach(photosToDelete, id: \.self){ asset in
                        AssetThumbnailView(asset: asset, onRemove: {
                            if let index = photosToDelete.firstIndex(of: asset){
...
struct AssetThumbnailView: View {
    ...
    private func loadThumbnail(){
        ...
        imageManager.requestImage(for: asset, targetSize: CGSize(width: 100, height: 100), contentMode: .aspectFill, options: options) { result, info in
            if let result = result {
                DispatchQueue.main.async{
                    self.image = result
                }
            }else{
                print("Erreur lors du chargement de l'image : \(String(describing: info))")
            }
